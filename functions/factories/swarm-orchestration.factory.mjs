...
export class SwarmOrchestrationFactory extends OrchestrationAdapter {
    ...
    async functionsInstanceCreate(project, envs, dryRun) {
        const commands = [
            ...
            `--env \"DATABASE_URI=mongodb://${project.parse.appId.toString().replace(new RegExp('[-]', 'ig'), '').trim()}:${project.parse.masterKey.toString().replace(new RegExp('[-]', 'ig'), '').trim()}@y4chub.ml:27017/${project.projectId}?authSource=admin\"`,
            "--env \"PORT=3000\"",
            "--env \"PRODUCTION=1\"",
    ...
    async databaseInstanceCreate(project, envs , dryRun) {
        const commands = [
            ...
            "--label \"traefik.http.routers.${projectId}-daas.tls=true\"",
            "--label \"traefik.http.routers.${projectId}-daas.tls.certresolver=le\"",
            "--env \"APPLICATION_ID=${appId}\"",
            `--env \"WEB_3_TOKEN=${process.env.WEB_3_TOKEN}\"`,
            "--env \"PROJECT_ID=${projectId}\"",
            "--env \"MASTER_KEY=${masterKey}\"",
