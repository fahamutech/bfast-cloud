...
export class SwarmOrchestrationFactory extends OrchestrationAdapter {
    ...
    async functionsInstanceCreate(project, envs, dryRun) {
        const commands = [
            ...
            `--env \"MONGO_URL=mongodb://${project.parse.appId.toString().replace(new RegExp('[-]', 'ig'), '').trim()}:${project.parse.masterKey.toString().replace(new RegExp('[-]', 'ig'), '').trim()}@139.162.206.182:27017/${project.projectId}?authSource=admin\"`,
            "--env \"PORT=3000\"",
            "--env \"PRODUCTION=1\"",
