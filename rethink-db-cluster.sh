...
docker service create --constraint node.role==manager --mount type=volume,src=rethinkdb,dst=/data,volume-driver=local --name rdb-primary --publish 81:8080 --network bfastweb rethinkdb:2.4.0 rethinkdb --bind all --initial-password auto
