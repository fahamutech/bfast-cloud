...
docker service create --mount type=volume,src=rethinkdb,dst=/data,volume-driver=rexray/s3fs --name rdb-primary --publish 81:8080 --network bfastweb rethinkdb:2.4.0 rethinkdb --bind all  --initial-password auto
